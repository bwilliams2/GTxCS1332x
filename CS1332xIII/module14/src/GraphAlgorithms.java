...
public class GraphAlgorithms {
    ...
    public static <T> Set<Edge<T>> prims(Vertex<T> start, Graph<T> graph) {
    	...
    	Queue<Edge<T>> edgeQueue = new PriorityQueue<Edge<T>>();
    	...
    	while (visited.size() != graphSize && !edgeQueue.isEmpty()) {
    		Edge<T> minEdge = edgeQueue.remove();
    		if (MST.contains(minEdge)) {
    			continue;
    		}
    		Vertex<T> nextVert;
    		if (!visited.contains(minEdge.getV())) {
    			nextVert = minEdge.getV();
    		} else {
    			continue;
    		}
    		visited.add(nextVert);
    		MST.add(minEdge);
			MST.add(new Edge<T>(minEdge.getV(), minEdge.getU(), minEdge.getWeight()));
        	for (Edge<T> edge : edges) {
        		if (!MST.contains(edge)) {
            		if (edge.getU() == nextVert && !visited.contains(edge.getV())) {
            			edgeQueue.add(edge);
            		}
        		}
        	}
    	}
    	if (MST.size()/2 < graphSize - 1) {
    		MST = null;
    	}
    	return MST;
    }
